...
def get_availability(country='BRA'):
    x = urlopen(
        'https://fwctickets.fifa.com/TopsAkaCalls/Calls.aspx/getRefreshChartAvaDem?l=en&c={}'.format(country)).read()
    obj = json.loads(x)
...
def get_basic_info():
    x = urlopen(
        'https://fwctickets.fifa.com/TopsAkaCalls/Calls.aspx/getBasicData?l=en&c=BRA').read()
    obj = json.loads(x)
    obj2 = json.loads(obj['d']['data'])
    return obj2
...
def get_products():
    obj2 = get_basic_info()
    products = dict([(x['ProductId'], x)
                    for x in obj2['BasicCodes']['PRODUCTS']])
    return products
