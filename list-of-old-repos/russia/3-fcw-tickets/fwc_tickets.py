...
matches_tocheck = ["IMT23", "IMT39", "IMT52",
                   "IMT07"]  # List of matches to check FIXME
...
def get_tickets_available():
    ...
    tix_data = requests.get(
        "https://tickets.fifa.com/API/WCachedL1/en/BasicCodes/GetBasicCodesAvailavilityDemmand?currencyId=USD")
    tix = tix_data.text
    tx = json.loads(tix)
    data = tx["Data"]
    ...
    for match in matches_tocheck:
        matches_availability.append(
            [el for el in avail if el["p"] == match][0:3])
